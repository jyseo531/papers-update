...
def insert_hf_news(news_data, db_path="db/database.db"):
    """
    Hugging Face 소식 데이터를 데이터베이스에 삽입
    """
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    ...
    for news in news_data:
        ...
        if row:
            ...
            cursor.execute("""
                ...
                WHERE id = ?
            """, (news.get("arxiv_links", "NULL"), news["github_url"], news["license"], news["category"], row[0]))
        else:
            # 데이터 삽입
            cursor.execute("""
                INSERT INTO hf_news (title, paper, github_url, license, category)
                VALUES (?, ?, ?, ?, ?)
            """, (news["title"], news.get("arxiv_links", "NULL"), news["github_url"], news["license"], news["category"]))

    conn.commit()
    conn.close()
    print(f"{len(news_data)}개의 소식 중 새롭게 저장되거나 업데이트된 항목이 있습니다!")
